...
import { deleteImage, getPublicIdFromUrl } from './cloudinary';
...
export async function deleteUsersByIds(ids = [], actingAdminEmail) {
    ...
    // Fetch targets first to cleanup any Cloudinary images
    const targets = await db
        .collection('admins')
        .find({ _id: { $in: objectIds } }, { projection: { image: 1, imagePublicId: 1 } })
        .toArray();

    // Delete images best-effort (do not block on failures)
    await Promise.all(
        targets.map(async (t) => {
            const pid = t?.imagePublicId || getPublicIdFromUrl(t?.image);
            if (pid) {
                try { await deleteImage(pid); } catch (_) {}
            }
        })
    );
